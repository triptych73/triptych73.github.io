...
class GraphClient:
    ...
    def get_drive_root_children(self):
        ...
        response = requests.get(f"{GRAPH_URI}/me/drive/root/children", headers=self.headers)
        if response.status_code == 200:
    ...
    def get_drive_item_children(self, item_id):
        ...
        response = requests.get(f"{GRAPH_URI}/me/drive/items/{item_id}/children", headers=self.headers)
        if response.status_code == 200:
            return response.json().get('value', [])
        return []

...
OUTPUT_DIR = 'assets/data/thumbnails_debug'
MAP_FILE = 'assets/data/thumbnail_map_debug.json'
...
with zipfile.ZipFile(INPUT_FILE, 'r') as z:
    ...
    for sheet in wb_root.findall('.//main:sheet', NS):
        ...
        name = sheet.get('name')
        print(f"[SHEET MAP] rId {r_id} -> '{name}'")
        sheets[r_id] = name
    ...
    for rel in rels_root.findall('.//{http://schemas.openxmlformats.org/package/2006/relationships}Relationship'):
        ...
        if r_id in sheets:
            ...
            print(f"[PATH MAP] '{sheets[r_id]}' -> xl/{target}")
        
    # 3. Process Each Sheet
    for sheet_name, sheet_path in sheet_paths.items():
        print(f"\n[PROCESSING] Sheet: '{sheet_name}' (path: {sheet_path})")
        ...
        if drawing is None:
            print("  No drawing element found")
            continue
        ...
        if sheet_rels not in z.namelist():
            print(f"  No rels file at {sheet_rels}")
            continue
        ...
        if not drawing_path:
            print("  Could not resolve drawing path")
            continue
        ...
        try:
            ...
            print(f"  ERROR reading drawing: {e}")
            continue
        ...
        # Find Anchors
        if sheet_name not in image_map:
            image_map[sheet_name] = {}
            ...
            if row_node is not None and pic_node is not None:
                ...
                if blip is not None:
                    ...
                    if embed in blip_map:
                        ...
                        # Save Image using the CORRECT sheet_name
        ...
        print(f"  Saved {saved_count} images with sheet name '{sheet_name}'")
...
print(f"Keys in map: {list(image_map.keys())}")
